...
class ProfitHunter:
    ...
    async def scan_new_tokens(self):
        """Scan for new token launches with potential"""
        while True:
            try:
                # Get new tokens from multiple DEXes
                new_tokens = await self.get_new_listings()
                
                for token in new_tokens:
                    # Quick initial filter
                    if not self.basic_token_check(token):
                        continue
                    
                    # Detailed analysis
                    analysis = await self.analyze_token_potential(token)
                    if analysis['score'] >= 80:  # High potential
                        self.opportunities['new_tokens'].append({
                            'token': token,
                            'analysis': analysis,
                        ...
                        # If very promising, act quickly
                        if analysis['score'] >= 90:
                            await self.execute_opportunity(token, 'new_launch')
